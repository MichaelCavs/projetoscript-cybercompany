...
class usuario extends db {
    ...
    public function cadastrar($dados = array()) {
        ...
        $q = $this->fetch("SELECT * FROM professores WHERE email = '$email'"); // Verifica se o usuário já está cadastado
        if (!$q) {
            $q = $this->query("INSERT INTO professores VALUES('', '$nome', '$sobrenome', '$cidade', '$estado', '$cep', '$telefone', '$email', '$senha', '$dateTime')");
            if ($q) {
                $erro['sucesso'] = true;
                return $erro;
            } else {
    ...
    public function editar($dados = array()) {
        ...
        $q = $this->fetch("SELECT * FROM professores WHERE email = '$email' AND idusuario <> $idusuario"); // Verifica se o email já existe
        if (!$q) {
            $q = $this->query("UPDATE professores SET nome = '$nome', sobrenome = '$sobrenome', cidade = '$cidade', estado = '$estado', cep = '$cep', telefone = '$telefone', email = '$email', senha = '$senha' WHERE idusuario = $idusuario");
            if ($q) {
                $erro['sucesso'] = true;
                return $erro;
            } else {
    ...
    public function deletarUsuario($id) {
        $q = $this->query("DELETE FROM professores WHERE idusuario = " . (int) $id);
        if ($q)
            return true;
        else
            return false;
    ...
    public function validaLogin($dados = array()) {
        ...
        $senha = sha1($senha);
        $q = $this->fetch("SELECT * FROM professores WHERE email = '$email' AND senha = '$senha'");
        if ($q) {
            session_start();
    ...
    public function usuarioInfo($id) {
        $dados = $this->fetch("SELECT * FROM professores WHERE idusuario = " . (int) $id);
        if ($dados)
            return $dados[0];
        else
            return null;
    ...
    public function todosUsuariosInfo() {
        $dados = $this->fetch("SELECT * FROM professores");
        if ($dados)
            return $dados;
        else
            return null;
